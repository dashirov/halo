...
- [ ] Add a [Unit Test](https://github.com/EqualExperts/dbt-unit-testing) to the
      new models.
...
`cd /workspace/acme-corp && dbt run` (`dbt run` from the `/workspace/acme-corp`
...
- `/workspace/acme-corp` has commands to run and test `dbt` models as well as
  lint (check) them for style.  We have also included some unit tests in the
  `tests/unit/` path which are executed as part of `dbt test`.
...
If you have any difficulties please consult the troubleshooting section below or
reach out to us and we will do our best to unstick you.

#### Troubleshooting

When running `cd /workspace/acme-corp && dbt run`, if you get errors which resemble
the following this indicates the database is not setup correctly.  Running
`cd /workspace && make reset-source-db` should fix the database for you.
